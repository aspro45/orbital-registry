...
def _norm_review(raw) -> dict:
    ...
    if summary == "":
        summary = "Orbital Registry review outcome: " + outcome
    if rationale == "":
        rationale = summary
...
def _review_prompt(protocol: str, case: dict, evidence_text: str) -> str:
    return (
        "You are Orbital Registry V2, a GenLayer source-consensus contract for Entity designations orbit through source-backed verification, orbital evidence trails, challenges and appeals.\n" + SECURITY +
...
def _ruling_prompt(mode: str, case: dict, filing: dict, evidence_text: str) -> str:
    return (
        "You are Orbital Registry V2 resolving a " + mode + " filing.\n" + SECURITY +
        "\nCase JSON: " + json.dumps(case, sort_keys=True) +
...
class OrbitalRegistry(gl.Contract):
    ...
    def __init__(self) -> None:
        self.clock = 0
        self.protocol = "Orbital Registry cases require public sources, prompt-injection resistance, source/claim comparison, challenge rights, appeal rights and visible audit trails."
    ...
    def _evidence_text(self, case: dict) -> str:
        out = "[primary " + case["sourceUrl"] + "]\n" + self._render(case["sourceUrl"], 360) + "\n\n"
    ...
    def review_with_genlayer(self, case_id: str) -> str:
        ...
        try:
            res = json.loads(leader())
        except Exception:
            res = _norm_review({"outcome": "unclear", "confidenceBps": 5200, "supportBps": 5000, "contradictionBps": 1200, "summary": "GenLayer review attempted; fallback stored because nondeterministic source execution was unavailable.", "rationale": "The contract preserves lifecycle/audit state instead of accepting a finalized transaction with no review row.", "riskFlags": ["GENLAYER_FALLBACK"]})
    ...
    def resolve_challenge_with_genlayer(self, case_id: str, challenge_id: str) -> None:
        ...
        txt = self._render(ch["evidenceUrl"], 260)
        public_case = self._public_case(case)
        def leader() -> str:
        ...
        try:
            ...
            res = _norm_ruling({"ruling": "inconclusive", "confidenceDeltaBps": 0, "reason": "GenLayer challenge resolver attempted; fallback stored because nondeterministic execution was unavailable.", "riskFlags": ["GENLAYER_FALLBACK"]}, "challenge")
    ...
    def resolve_appeal_with_genlayer(self, case_id: str, appeal_id: str) -> None:
        ...
        txt = self._render(ap["evidenceUrl"], 260)
        public_case = self._public_case(case)
        def leader() -> str:
        ...
        try:
            ...
            res = _norm_ruling({"ruling": "inconclusive", "confidenceDeltaBps": 0, "reason": "GenLayer appeal resolver attempted; fallback stored because nondeterministic execution was unavailable.", "riskFlags": ["GENLAYER_FALLBACK"]}, "appeal")
    ...
    def get_contract_stats(self) -> str:
        ...
        while i < len(self.cases):
            ...
            if case.get("outcome") == "supported":
                ...
                unclear += 1
            i += 1
        return json.dumps({"contract": "Orbital Registry V2", "cases": len(self.cases), "evidence": len(self.evidence),
    ...
    def get_frontend_bootstrap(self) -> str:
        return json.dumps({"contract": "Orbital Registry V2", "statuses": list(STATUSES), "outcomes": list(OUTCOMES),
                           "legacyNoun": "object", "recentCases": json.loads(self.get_recent_cases(10)),
    ...
    def register_object(self, designation: str, source_url: str) -> int:
        fields = {}
        fields["legacyType"] = "object"
        return int(self._create_case("object", designation, designation, source_url, fields))

    @gl.public.write
    def verify_object(self, item_id: int) -> None:
        case = self._load_case(str(item_id))
        if case.get("outcome") == "pending":
            self.review_with_genlayer(str(item_id))

    @gl.public.write
    def archive_object(self, item_id: int) -> None:
    ...
    @gl.public.view
    def get_object(self, item_id: int) -> dict:
        case = self._load_case(str(item_id))
        fields = case.get("fields", {})
        return {
            "registrar": case.get("actor", ""),
            "designation": case.get("title", ""),

            "designation": case.get("claim", ""),
            "source_url": case.get("sourceUrl", ""),
            "status": self._legacy_status(case),
            "assessment": case.get("summary", ""),
        ...
        while i < len(self.cases):
            ...
            if case.get("status") != "ARCHIVED":
                ...
                if code == 1:
                    ...
                    pending += 1
            i += 1
        out = {"total": pos + neg + pending + uncertain, "settled": pos, "rejected": neg, "pending": pending}
        out["holding"] = uncertain
        return out
